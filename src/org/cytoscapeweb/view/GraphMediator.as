        ...
        private function onDragNode(evt:DragEvent):void {
            ...
            for each (n in nodes) {
                ...
                if (n is CompoundNodeSprite) {
					...
					// SBGN MODIFICATION
					// when dragged also update "state variable" and "unit of information" compound node sprites
					// since they are not rendered as node
					var stateAndIfoGlyphs:Array = ns.data.stateAndInfoGlyphs;
					if (stateAndIfoGlyphs != null) 
					{
						for each (var i:CompoundNodeSprite in stateAndIfoGlyphs) 
						{
							i.x += amountX;
							i.y += amountY;
						}
